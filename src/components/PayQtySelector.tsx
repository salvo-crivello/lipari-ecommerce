...
function PayQtySelector({ quantity, setQuantity, maxQty = 100, handleAddToCart }: QuantityInputProps) {
	...
	return (
		<div className="relative" ref={dropdownRef}>
			<Button
				...
				className="rounded-md min-w-32 relative !px-2"
				aria-haspopup="listbox"
				aria-expanded={isOpen}
			>
				<span className="text-neutral-300 w-full text-left">{`Q'ty`}</span>
				{quantity}
				<ChevronDown
					size={'1.2em'}
					className={clsx('shrink-0 text-neutral-500 transition-all duration-150 ease-in', {
						'rotate-180': isOpen,
					})}
				/>
			</Button>
