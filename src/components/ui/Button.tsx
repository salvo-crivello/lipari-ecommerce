...
export const buttonVariants = cva(
	'group border inline-flex gap-2.5 items-center font-medium transition-all duration-150 ease-in h-fit w-fit text-nowrap disabled:pointer-events-none text-md disabled:opacity-30 leading-none',
	{
		variants: {
			variant: {
				primary:
					'bg-blue-200 border-blue-200 text-blue-800 hover:bg-blue-300 active:bg-blue-400 hover:-translate-y-1 active:-translate-y-0 justify-center',
				secondary: 'border-blue-200 text-blue-800 hover:bg-blue-50 hover:border-blue-300 justify-center',
				ghost: 'justify-center bg-blue-50 border-blue-50 hover:bg-blue-100 active:bg-blue-50 text-blue-800',
				with_border:
...
export const Button = ({
	...
	return (
		<button
			...
			{...props}
		>
			{Icon && (iconPos === 'left' || iconPos === 'center') && <Icon size="1.2em" className="shrink-0" />}
			{children}
			{Icon && iconPos === 'right' && <Icon size="1.2em" className="shrink-0" />}
		</button>
	);
};
...
interface IconButtonProps extends ButtonHTMLAttributes<HTMLButtonElement>, VariantProps<typeof buttonVariants> {
...
export const IconButton = ({
	...
	return (
		<button className={clsx('w-fit', buttonVariants({ variant, size, rounded }), className)} {...props}>
			{children}
			{Icon && <Icon size={children ? '1.2em' : '1.2rem'} className="shrink-0" />}
		</button>
	);
};
...
const alotButtonIconVariant = cva('shrink-0 transition-all group-hover:-rotate-[20deg] duration-200 ease-in');
...
export const AlotButton = ({
	...
	return (
		<button className={clsx('relative inline-block group disabled:pointer-events-none', className)} {...props}>
			{/* Static shadow base */}
			...
			>
				{Icon && iconPos === 'left' && <Icon size="1.2em" className={clsx(alotButtonIconVariant())} />}
				{children}
				{Icon && iconPos === 'right' && <Icon size="1.2em" className={clsx(alotButtonIconVariant())} />}
			</span>
		</button>
	);
};
