...
const inputVariants = cva("w-full", {
  variants: {
    variant: {
      default:
        "flex h-9 w-full rounded-md border border-neutral-300 bg-transparent px-3 py-1 transition-colors file:border-0 file:bg-transparent file:text-sm file:font-medium file:text-foreground placeholder:text-neutral-300 focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-blue-700 group-hover:ring-1 group-hover:ring-blue-700 disabled:cursor-not-allowed disabled:opacity-50 md:text-sm",
    },
    sizes: {
      sm: "text-xs p-2",
      md: "text-sm p-2",
...
export type LabelProps = LabelHTMLAttributes<HTMLLabelElement> &
  LabelVariants & { required?: boolean };
...
const InputGroup = ({
  ...
  return (
    <InputGroupProvider value={{ sync }}>
      <div
        className={clsx(
          "w-full bg-background-soft h-fit flex flex-col group",
          className
        )}
      >
        {children}
...
const Label = forwardRef<HTMLLabelElement, LabelProps>(function LabelComponent(
  ...
  return (
    <label
      ref={ref}
      className={clsx(labelVariants({ variant, className, size }))}
      {...props}
    >
      {children}
      {required && "*"}
    ...
    return (
      <div className="relative w-full">
        ...
        <input
          ref={ref}
          className={clsx(
            inputVariants({ variant, sizes, rounded, border }),
            className,
            {
  ...
  function FormTextareaComponent(
    ...
    return (
      <textarea
        ...
        className={clsx(
          "relative w-full",
          textareaVariants({ variant, sizes, rounded, border }),
          className
        )}
        {...props}
      />
...
const Message = forwardRef<
  HTMLElement,
  HTMLAttributes<HTMLSpanElement> & MessageVariants
>(function MessageComponent(
  ...
  return (
    <span
      ref={ref}
      className={clsx(messageVariants({ variant, className, size }))}
      {...props}
    >
      ...
      render={({ field: { onChange, value } }) => (
        <div className="flex items-center max-sm:items-start gap-4 group">
          ...
          <label
            ...
            className="text-2xs leading-snug text-pretty text-foreground-soft"
          >
            {label}
            <Link
              href={linkUrl}
