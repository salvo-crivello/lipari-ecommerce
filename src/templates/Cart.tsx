...
function Cart() {
  const {
    ...
    isOpen: cartIsOpen,
    totalCost,
  } = useSelector((state: RootState) => state.cartData);

  // const cartLength = useMemo(() => items.length, [items]);
  ...
  return (
    <>
      ...
      <motion.div
        className={clsx(
          "w-full fixed inset-0 h-screen z-[9000]",
          "bg-[url('/hero.webp')] bg-cover bg-top"
        )}
        initial={{ y: cartIsOpen ? "0%" : "-100%" }}
        animate={cartIsOpen ? { y: cartIsOpen ? "0%" : "-100%" } : undefined}
        transition={{
