...
function Product() {
	...
	const product = useSelector((state: RootState) => {
		...
		return productsArray.find((p) => p.id === Number(id));
	});
	...
	if (!product)
		return (
			<header className="h-screen px-5 lg:px-10 pt-40 pb-20 flex flex-col relative max-w-7xl mx-auto">
				...
				<p className="text-neutral-500">Il prodotto che stai cercando non esiste o è stato rimosso.</p>
			</header>
		);

	return (
		<article className="px-2 sm:px-5 lg:px-10 max-w-7xl flex gap-5 lg:gap-10 mx-auto">
			<div>
