...
const CartPayment = ({
  ...
  return (
    <Card className="w-full sm:min-w-[300px] sm:max-w-[300px] rounded-lg flex flex-col items-start bg-white shadow-xl shadow-neutral-100 border border-neutral-100 h-fit">
      ...
      <CardContent className="w-full text-sm font-medium">
        <p className="flex justify-between items-center">
          <span className="text-neutral-500 font-normal">{`${cartNr} prodott${
            cartNr > 1 ? "i" : "o"
          }:`}</span>
        ...
        <p className="flex justify-between items-center">
          ...
          <span
            className={clsx({ "text-lime-600": promoCode && promoCode.valid })}
          >
            {promoCode && promoCode.valid
              ? `-${currencyFormat(promoDiscount)}`
              : `${currencyFormat(0)}`}
          </span>
        </p>
        <p className="flex justify-between items-center text-md mt-2 pt-2 border-t border-neutral-200">
